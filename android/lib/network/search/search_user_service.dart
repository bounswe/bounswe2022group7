import 'dart:convert';
import 'package:flutter/foundation.dart';
import 'package:http/http.dart';

import 'package:android/config/api_endpoints.dart';
import 'package:android/network/search/search_outputs.dart';

import '../../models/user_model.dart';
import '../../shared_prefs/user_preferences.dart';

Future<SearchUserOutput> searchUserNetwork(String keys) async {
  String request = "keywords=${keys.trim().replaceAll(' ', '&keywords=')}";
  SearchUserOutput seo;

  Response response;
  try {
    CurrentUser? user = await getUser();
    final token = user == null ? "" : user.token;
    response = await get(
      Uri.parse(Uri.encodeFull('$searchUserURL?$request')),
      headers: {
        'Content-Type': 'application/json; charset=UTF-8',
        'Authorization': 'Bearer $token'
      },
    );

    if (response.statusCode != 200) {
      return SearchUserOutput(status: response.statusCode.toString());
    }

    Iterable data = json.decode(response.body);
    seo = await searchuserOutputJsonConverter(data);
  } catch (err) {
    print("error $err");
    return SearchUserOutput(status: err.toString());
  }

  print(seo.userSearch?.userList.toString());
  return seo;
}
